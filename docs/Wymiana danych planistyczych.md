...
| Nadawca | Obiekt generacji lub poboru albo obiekt wymiany | Aktywność na rynku bilansującym | Rodzaj przekazywanych danych | Typy dokumentów |
|-----------| :---------: | :---------: |-----------|-----------|
...
* saldo wymiany - /pwdp/api/v1/exchange-schedule-submissions
...
W kanale B2B oczekiwane jest zgłaszanie kompletnych danych planistycznych. Dotyczy to zarówno danych o charakterze okresowym (postoje, ubytki, niesprawności układu regulacji) jak i danych grafikowych. Jednocześnie niektóre grafiki (serie danych), np. w planie pracy albo w poleceniach ruchowych OSD mogą nie mieć określonych wartości w całym zadanym horyzoncie. Z tego powodu dopuszczalne jest podawanie w ramach takiego grafiku wartości pustych (null) oznaczających brak określonej wartości. 

Ponadto kompletność danego zgłoszenia nie oznacza podawania wszystkich możliwych dla danego komunikatu serii danych - tylko niektóre z nich są wymagane w każdym zgłoszeniu, pozostałe są uzupełniające. Dokładnie jest opisane w tabeli poniżej.

| Kod typu dokumentu (pole type) | Rodzaj komunikatu | Kod szczegółowy (pole businessType) | Rodzaj danych | Seria danych wymagana | Kompletność serii danych <br /><br /> Dopuszczalne puste wartości (null) |
|-----------|-----------|-----------|-----------|-----------|-----------|
| POS | Postój | POS | Postój | Nie dotyczy | Nie dotyczy |
| UBT | Ubytek | UBTD | Ubytek dodatni | Nie dotyczy | Nie dotyczy |
| |  | UBTU | Ubytek ujemny | Nie dotyczy | Nie dotyczy |
| NUR | Niesprawność układu regulacji | FCRG | Niesprawność układu regulacji FCR_G - rezerwa utrzymania częstotliwości w górę (FCRG, regulacja pierwotna) | Nie dotyczy | Nie dotyczy |
 | |  | FCRD | Niesprawność układu regulacji FCR_D - rezerwa utrzymania częstotliwości w dół (FCRD, regulacja pierwotna) | Nie dotyczy | Nie dotyczy |
 | |  | AFRG | Niesprawność układu regulacji aFRR_G - rezerwa odbudowy częstotliwości z aktywacją automatyczną w górę (aFRRG, regulacja wtórna) | Nie dotyczy | Nie dotyczy |
 | |  | AFRD | Niesprawność układu regulacji aFRR_D - rezerwa odbudowy częstotliwości z aktywacją automatyczną w dół (aFRRD, regulacja wtórna) | Nie dotyczy | Nie dotyczy |
 | |  | MFRG | Niesprawność układu regulacji mFRRd_G - rezerwa odbudowy częstotliwości z aktywacją nieautomatyczną typu bezpośredniego w górę (mFRRdG, regulacja wtórna) | Nie dotyczy | Nie dotyczy |
 | |  | MFRD | Niesprawność układu regulacji mFRRd_D - rezerwa odbudowy częstotliwości z aktywacją nieautomatyczną typu bezpośredniego w dół (mFRRdD, regulacja wtórna) | Nie dotyczy | Nie dotyczy |
 | |  | ARNE | Niesprawność układu regulacji ARNE - zdolność regulacji mocy biernej  | Nie dotyczy | Nie dotyczy |
 | |  | EST | Niesprawność układu regulacji Estymata - zdolność do szacowana w sposób ciągły w czasie rzeczywistym wartości mocy czynnej możliwej do generacji przez zasób typu OZE, przy uwzględnieniu dostępności energii wiatru lub promieniowania słonecznego oraz warunków eksploatacyjnych | Nie dotyczy | Nie dotyczy |
| HRM | Harmonogram dyspozycyjności | MINE | Harmonogram dyspozycyjności - Możliwe minimum dyspozycyjności elektrownianej | Tak <br /><br />(w każdym istniejącym kierunku) | Kompletne |
| |  | MAXE | Harmonogram dyspozycyjności - Dostępne maksimum dyspozycyjności elektrownianej | Tak <br /><br /> (w każdym istniejącym kierunku) | Kompletne |
| |  | MINS | Harmonogram dyspozycyjności - Możliwe minimum dyspozycyjności sieciowej | Tak <br /><br /> (w każdym istniejącym kierunku) | Kompletne |
| |  | MAXS | Harmonogram dyspozycyjności - Dostępne maksimum dyspozycyjności sieciowej | Tak <br /><br /> (w każdym istniejącym kierunku) | Kompletne |
| PUZ | Pojemność użytkowa (magazynu) | PUZ | Pojemność użytkowa (magazynu) | Tak | Kompletne |
| PLP | Plan pracy | MCSR | Plan pracy - średnie wielkości mocy czynnej netto wprowadzanej do sieci/pobieranej z sieci w poszczególnych OREB | Tak <br /><br /> (w każdym istniejącym kierunku) | Kompletne |
| |  | MCKN | Plan pracy - wielkości mocy czynnej netto wprowadzanej do sieci/pobieranej z sieci na koniec poszczególnych OREB | Tak <br /><br /> (w każdym istniejącym kierunku) | Kompletne |
| |  | MCSO | Plan pracy - średnie wielkości mocy czynnej netto wprowadzanej do sieci/pobieranej z sieci w poszczególnych OREB z uwzględnieniem polecenia ruchowego OSD | Nie | Mogą być puste |
| |  | MCKO | Plan pracy - wielkości mocy czynnej netto wprowadzanej do sieci/pobieranej z sieci na koniec poszczególnych OREB z uwzględnieniem polecenia ruchowego OSD | Nie | Mogą być puste |
| |  | ZNSR | Plan pracy - znacznik true/false określający, czy wielkości generacji mocy czynnej wprowadzone do sieci w poszczególnych OREB przez MWE odzwierciedlają generację mocy czynnej netto odpowiadającą potencjałowi generacji MWE wynikającemu z warunków meteorologicznych <br /><br /> (dotyczy tylko MWE typu B, C, D, dane w kierunku generacji) | Tak | Kompletne |
...
| |  | AGSR | Plan pracy - średnie wielkości generacji mocy czynnej netto wytwarzanej w danym OREB przez MWE na własne potrzeby (dla MWE ze znacznikiem „autogeneracja”) <br /><br /> (dotyczy tylko MWE typu B, C, D, dane w kierunku generacji) | Dla AG=T → Tak <br /><br /> Dla AG=N → Nie | Kompletne |
| |  | AGKN | Plan pracy - wielkości generacji mocy czynnej netto wytwarzane na koniec poszczególnych OREB przez MWE na własne potrzeby (dla MWE ze znacznikiem „autogeneracja”) <br /><br /> (dotyczy tylko MWE typu B, C, D, dane w kierunku generacji) | Dla AG=T → Tak <br /><br /> Dla AG=N → Nie | Kompletne |
| |  | MCSD | Plan pracy - średnie wielkości generacji mocy czynnej netto MWE w poszczególnych OREB, określające potencjał generacji MWE wynikający z warunków meteorologicznych i z mocy dyspozycyjnej elektrownianej MWE <br /><br /> (dotyczy tylko MWE PPM - w kategorii FW, PV lub układ hybrydowy, w tym dla poszczególnych elementów tego układu, dane w kierunku generacji) | Dla PPM → Tak | Kompletne |
| |  | MCKD | Plan pracy - wielkości generacji mocy czynnej netto MWE na koniec poszczególnych OREB, określające potencjał generacji MWE wynikający z warunków meteorologicznych i z mocy dyspozycyjnej elektrownianej MWE (dotyczy tylko PPM typu FW, PPM typu PV i PPM typu układ hybrydowy, w tym dla poszczególnych elementów tego układu) <br /><br /> (dotyczy tylko MWE PPM - w kategorii FW, PV lub układ hybrydowy, w tym dla poszczególnych elementów tego układu, dane w kierunku generacji) | Dla PPM → Tak | Kompletne |
| |  | MCSZ | Plan pracy - średnie wielkości generacji mocy czynnej netto MWE w poszczególnych OREB, określające potencjał generacji MWE wynikający z warunków meteorologicznych i z mocy zainstalowanej MWE (dotyczy tylko PPM typu FW, PPM typu PV i PPM typu układ hybrydowy, w tym dla poszczególnych elementów tego układu) <br /><br /> (dotyczy tylko MWE PPM - w kategorii FW, PV lub układ hybrydowy, w tym dla poszczególnych elementów tego układu, dane w kierunku generacji) | Dla PPM → Tak | Kompletne |
...
| |  | PWP | Plan pracy - znacznik (0, 1, 2) planowanej pracy wymuszonej, dla MEE w trybie rozładowania <br /><br /> (dotyczy tylko MWE typu D oraz MEE w sieci OSP, dane w kierunku generacji) | Nie | Kompletne |
| |  | SNAL | Stan naładowania MEE na koniec poszczególnych OREB <br /><br /> (dotyczy tylko MEE) | Dla MEE → Tak | Kompletne |
| PRU | Polecenia ruchowe OSD | OMAX | Polecenia ruchowe OSD - Pmax wielkości mocy czynnej netto w poszczególnych OREB wynikający z poleceń ruchowych OSDp | Nie <br /><br /> (konieczna dla zgłoszenia jest jedna z serii) | Mogą być puste |
| |  | OMIN | Polecenia ruchowe OSD - Pmin wielkości mocy czynnej netto w poszczególnych OREB wynikający z poleceń ruchowych OSDp | Nie <br /><br /> (konieczna dla zgłoszenia jest jedna z serii) | Mogą być puste |
| WYM | Saldo wymiany | WYM | Saldo wymiany | Tak | Kompletne |
...
Z kolei jeśli we wcześniejszym zgłoszeniu przekazano określoną serię danych, a w kolejnym zgłoszeniu w ogóle ta seria danych nie występuje (a nie jest wymagana), to wartości z poprzednio podanej serii pozostają - nie są usuwane.
